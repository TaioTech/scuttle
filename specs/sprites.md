...
| 7 | Should the frisbee be aimed at the crab's row at all? | Shipped aimed: each throw targeted `lockRow ± 1`, re-aimed every 540 ticks at wherever the player had got to. Reported in play as "it keeps chasing me and I can't get past it." | **Cut, 2026-07-30.** Measured over 400 seeds, 67% of throws covered the lane the crab was standing in and 69% covered the lane it had to step into; the covered pair was never further than one lane away. It crosses at 56 board units/sec against the crab's 46, so sideways evasion is arithmetically impossible, and the crab cannot step back — the only escape was forward through the second lane it also covered. That is the *dog's* specified behaviour (`scuttle.md`: "a dog that tracks the player"), acquired without the readability rule this spec's own risk note demands of a pursuer. Aiming it was a real answer to a real problem — a frisbee thrown at a seeded row is one most runs never meet, since only 11 of 32 lanes are on screen — so the fix is not a one-line revert and both pursuers are now one decision. It returns with the dog, under a rule that commits to a direction for a beat or loses interest. `git show b5e50c4` has the flight arithmetic and its tests. |
